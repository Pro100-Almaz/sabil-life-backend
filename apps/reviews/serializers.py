...
from apps.reviews.services import can_review, can_review_tutor
...
class TutorReviewListSerializer(serializers.ModelSerializer):
    """
    Public read serializer for tutor reviews — safe to expose without auth.
    ...
    def get_author_name(self, obj: TutorReview) -> str:
        name = getattr(obj.author, "full_name", "") or ""
        return name.strip() or "Anonymous"


class TutorReviewCreateSerializer(serializers.Serializer):
    """
    Input serializer for POST /api/v1/tutors/{tutor_id}/reviews/.

    Validates:
    1. rating is 1-5.
    2. Engagement gate: family must have a CONTACTED / ACCEPTED / COMPLETED
       inquiry with this tutor (see services.can_review_tutor).
    3. Uniqueness: one review per (tutor, author). Converts IntegrityError
       to a 409-friendly ValidationError that the view catches.
    """
    ...
    def validate(self, attrs):
        ...
        if request and tutor and not can_review_tutor(request.user, tutor):
            raise serializers.ValidationError(
                {
                    "non_field_errors": [
                        "You can only review a tutor you have engaged with."
                    ]
                }
            )
        return attrs
    ...
    def create(self, validated_data):
        ...
        try:
            return TutorReview.objects.create(
                tutor=tutor,
                author=request.user,
                rating=validated_data["rating"],
                text=validated_data.get("text", ""),
...
class TutorReviewUpdateSerializer(serializers.ModelSerializer):
    """
    Input serializer for PATCH /api/v1/tutor-reviews/{id}/.
