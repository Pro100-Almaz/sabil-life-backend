...
def make_inquiry(status_val=InquiryStatus.PENDING):
    uid = id(object())
...
class TestInquiryStateMachine:
    ...
    def test_new_to_accepted(self):
        inq = make_inquiry(InquiryStatus.PENDING)
    ...
    def test_completed_to_new_raises(self):
        ...
        with pytest.raises(InvalidTransition):
            transition(inq, InquiryStatus.DECLINED, actor=inq.tutor.user)
    ...
    def test_accepted_to_new_raises(self):
        inq = make_inquiry(InquiryStatus.ACCEPTED)
        with pytest.raises(InvalidTransition):
            transition(inq, InquiryStatus.PENDING, actor=inq.tutor.user)
    ...
    def test_transition_saves_to_db(self):
        ...
        transition(inq, InquiryStatus.ACCEPTED, actor=inq.tutor.user)
        inq.refresh_from_db()
        assert inq.status == InquiryStatus.ACCEPTED
...
class TestTutorStatusUpdateEndpoint:
    ...
    def _make_inquiry(self, status_val=InquiryStatus.PENDING):
        return Inquiry.objects.create(
            family=self.family,
            tutor=self.tutor,
            message="Test",
